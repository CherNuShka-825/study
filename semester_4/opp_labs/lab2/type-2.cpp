...
vector<double> simpleIterationMethodOMPv2(
    ...
    #pragma omp parallel shared(x, xNew, converged, iterationsDone, normB, residualSum)
    {
        #pragma omp for reduction(+:normB2) schedule(runtime)
        for (int i = 0; i < N; ++i) {
            normB2 += b[i] * b[i];
        }
...
int main() {
    ...
    vector<double> x = simpleIterationMethodOMPv2(
        A,
        b,
        N,
        tau,
